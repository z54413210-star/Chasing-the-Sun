...
public class PlayerOneLevel3Controller2D : MonoBehaviour
{
    [Header("Movement")]
    ...
    private LadderZone _lastTopExitedLadder;
    ...
    private void Awake()
    {
        ...
        if (GetComponent<PlayerOneLevel3LadderTraversalAssist>() == null)
        {
            gameObject.AddComponent<PlayerOneLevel3LadderTraversalAssist>();
        }
    ...
    {
        NotifyEnterLadder(ResolveLadderZone(other));
    }

    private void OnTriggerExit2D(Collider2D other)
    {
        NotifyExitLadder(ResolveLadderZone(other));
    }
    ...
    {
        var ladderAssist = GetComponent<PlayerOneLevel3LadderTraversalAssist>();
    ...
    private bool TryGetTopReattachLadder(out LadderZone ladder)
    {
        ladder = null;
        ...
        for (var i = 0; i < hitCount; i++)
        {
            ...
            var candidate = hit.GetComponent<LadderZone>();
            if (candidate == null)
            {
                candidate = hit.GetComponentInParent<LadderZone>();
            }
    ...
    private bool IsTopReattachCandidate(LadderZone ladder)
    {
        if (ladder == null)
        {
            return false;
    ...
    private static LadderZone ResolveLadderZone(Collider2D other)
    {
        if (other == null)
        {
            return null;
        ...
        var ladderZone = other.GetComponent<LadderZone>();
        if (ladderZone != null)
        {
            return ladderZone;
        }

        return other.GetComponentInParent<LadderZone>();
    }
}
